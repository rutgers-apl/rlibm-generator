...
int main(int argc, char** argv) {
    ...
    PolySynth<bfloat16, Elementary<bfloat16>>  bf16cospi(SAMPLE, 1000);
    bf16cospi.SetUpSamplingOption(0, 0.5);
    bf16cospi.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        bf16cospi.ResetPolynomials();
        bf16cospi.FindPolynomialOnce({0});
        bf16cospi.FindPolynomialsUntilBefore({0, 2, 4, 6}, 0.5);
        bf16cospi.FindPolynomials({0});
        printf("\tCOMPLETED\n\n");
    } while (bf16cospi.TestAndAddSamplePoints());
