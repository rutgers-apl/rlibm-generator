...
int main(int argc, char** argv) {
    ...
    PolySynth<bfloat16, Elementary<bfloat16>> bf16log10(SAMPLE, 50);
    bf16log10.SetUpSamplingOption(0.75, 1.5);
    bf16log10.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        bf16log10.ResetPolynomials();
        bf16log10.FindPolynomials({1, 3, 5});
        printf("\tCOMPLETED\n\n");
    } while (bf16log10.TestAndAddSamplePoints());
    
    bf16log10.poly->PrintPiecewiseInfo();
    bf16log10.PerformErrorAnalysis();
    mpfr_clear(mval);
    return 0;
}
