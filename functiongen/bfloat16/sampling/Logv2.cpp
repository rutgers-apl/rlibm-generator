...
int main(int argc, char** argv) {
    ...
    PolySynth<bfloat16, Elementary<bfloat16>> bf16log(SAMPLE, 50);
    bf16log.SetUpSamplingOption(0.75, 1.5);
    bf16log.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        bf16log.ResetPolynomials();
        bf16log.FindPolynomials({1, 3, 5, 7});
        printf("\tCOMPLETED\n\n");
    } while (bf16log.TestAndAddSamplePoints());
