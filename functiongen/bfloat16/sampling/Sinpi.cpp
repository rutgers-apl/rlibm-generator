...
int main(int argc, char** argv) {
    ...
    PolySynth<bfloat16, Elementary<bfloat16>> bf16sinpi(SAMPLE, 1000);
    bf16sinpi.SetUpSamplingOption(0, 0.5);
    bf16sinpi.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        bf16sinpi.ResetPolynomials();
        bf16sinpi.FindPolynomialOnce({1});
        bf16sinpi.FindPolynomials({1, 3, 5, 7});
        printf("\tCOMPLETED\n\n");
    } while (bf16sinpi.TestAndAddSamplePoints());
