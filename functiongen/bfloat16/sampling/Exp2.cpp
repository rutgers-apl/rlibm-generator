...
int main(int argc, char** argv) {
    ...
    PolySynth<bfloat16, Elementary<bfloat16>> bf16exp2(SAMPLE, 200);
    bf16exp2.SetUpSamplingOption(0, 1);
    bf16exp2.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        bf16exp2.ResetPolynomials();
        bf16exp2.FindPolynomials({0, 1, 2, 3, 4});
        printf("\tCOMPLETED\n\n");
    } while (bf16exp2.TestAndAddSamplePoints());
