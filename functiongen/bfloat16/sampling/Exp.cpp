...
int main(int argc, char** argv) {
    ...
    PolySynth<bfloat16, Elementary<bfloat16>> bf16exp(SAMPLE, 200);
    bf16exp.SetUpSamplingOption(0, 0.694);
    bf16exp.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        bf16exp.ResetPolynomials();
        bf16exp.FindPolynomials({0, 1, 2, 3, 4});
        printf("\tCOMPLETED\n\n");
    } while (bf16exp.TestAndAddSamplePoints());
