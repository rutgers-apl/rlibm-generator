...
int main(int argc, char** argv) {
    ...
    PolySynth<bfloat16, Elementary<bfloat16>> bf16sqrt(SAMPLE, 200);
    bf16sqrt.SetUpSamplingOption(1, 4);
    bf16sqrt.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        bf16sqrt.ResetPolynomials();
        bf16sqrt.FindPolynomials({0, 1, 2, 3, 4});
        printf("\tCOMPLETED\n\n");
    } while (bf16sqrt.TestAndAddSamplePoints());
