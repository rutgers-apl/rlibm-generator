...
int main(int argc, char** argv) {
    ...
    PolySynth<bfloat16, Elementary<bfloat16>> bf16cbrt(SAMPLE, 200);
    bf16cbrt.SetUpSamplingOption(1, 8);
    bf16cbrt.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        bf16cbrt.ResetPolynomials();
        bf16cbrt.FindPolynomials({0, 1, 2, 3, 4, 5, 6});
        printf("\tCOMPLETED\n\n");
    } while (bf16cbrt.TestAndAddSamplePoints());
