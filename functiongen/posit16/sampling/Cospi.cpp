...
int main(int argc, char** argv) {
    ...
    PolySynth<posit16, Elementary<posit16>> p16cospi(SAMPLE, 1000);
    p16cospi.SetUpSamplingOption(0, 0.5);
    p16cospi.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        p16cospi.ResetPolynomials();
        p16cospi.FindPolynomialOnce({0});
        p16cospi.FindPolynomialsUntilBefore({0, 2, 4, 6, 8}, 0.5);
        p16cospi.FindPolynomials({0});
        printf("\tCOMPLETED\n\n");
    } while (p16cospi.TestAndAddSamplePoints());
