...
int main(int argc, char** argv) {
    ...
    PolySynth<posit16, Elementary<posit16>> p16Sinpi(SAMPLE, 1000);
    p16Sinpi.SetUpSamplingOption(0, 0.5);
    p16Sinpi.CalcIntervals();
    
    do {
        printf("FINDING POLYNOMIALS\n");
        p16Sinpi.ResetPolynomials();
        p16Sinpi.FindPolynomialOnce({1});
        p16Sinpi.FindPolynomials({1, 3, 5, 7, 9});
        printf("\tCOMPLETED\n\n");
    } while (p16Sinpi.TestAndAddSamplePoints());
